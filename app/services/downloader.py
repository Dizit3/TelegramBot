...
class TikTokDownloader(IVideoDownloader):
        ...
        try:
            async with aiohttp.ClientSession(headers=headers) as session:
                async with session.get(url, allow_redirects=True) as response:
                    ...
                    if (
                        "tiktok.com/?_r=1" in final_url
                        or final_url.strip("/") == "https://www.tiktok.com"
                    ):
                        raise TikTokBlockError(
                            "TikTok блокирует доступ к этой ссылке. Попробуйте прислать полную ссылку."
                        )
    ...
    async def download(
        ...
        if res:
            ...
            return VideoInfo(file_path="", image_paths=image_paths, title="TikTok Slideshow")
